...
#include "unitree_legged_msgs/msg/detail/led__rosidl_typesupport_fastrtps_c.h"
...
using _LED__ros_msg_type = unitree_legged_msgs__msg__LED;
...
static uint32_t _LED__get_serialized_size(const void * untyped_ros_message)
{
  return static_cast<uint32_t>(
    get_serialized_size_unitree_legged_msgs__msg__LED(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = unitree_legged_msgs__msg__LED;
    is_plain =
      (
      offsetof(DataType, b) +
      last_member_size
...
static size_t _LED__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_unitree_legged_msgs__msg__LED(
