...
bool unitree_legged_msgs__msg__motor_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // tauest
    PyObject * field = PyObject_GetAttrString(_pymsg, "tauest");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->tauest = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // q_raw
...
PyObject * unitree_legged_msgs__msg__motor_state__convert_to_py(void * raw_ros_message)
{
  ...
  {  // tauest
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->tauest);
    {
      int rc = PyObject_SetAttrString(_pymessage, "tauest", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
